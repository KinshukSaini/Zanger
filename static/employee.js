...
function openEditDialog() {
  ...
  dialogContainer.id = "ai-edit-dialog";
  dialogContainer.className = "ai-dialog-container";
  ...
  dialogContainer.innerHTML = `
    <div class="ai-dialog">
      ...
      <div class="ai-dialog-content">
        <textarea id="ai-edit-text">${selectedText}</textarea>
        <div class="ai-prompt-container">
          <input type="text" id="ai-prompt" placeholder="Enter instructions for AI edit...">
          <button id="ai-submit-btn" class="btn btn-primary">Submit</button>
        </div>
      </div>
    </div>
  `;
  ...
  // Add event listeners
  dialogContainer.querySelector(".close-btn").addEventListener("click", closeEditDialog);
  dialogContainer.querySelector("#ai-submit-btn").addEventListener("click", submitAIEditRequest);
}
...
function submitAIEditRequest() {
  ...
  const originalText = document.getElementById("ai-edit-text").value;
  
  if (!promptText || !originalText) {
    alert("Please provide both text to edit and instructions.");
    return;
  }
  ...
  setTimeout(() => {
    const newText = `[AI edited]: ${originalText}`;
    updateDocumentWithAIResponse(newText);
    closeEditDialog();
  }, 1000);
}
...
function updateDocumentWithAIResponse(newText) {
  ...
  // Update the document data structure
  // This would require more complex logic to update the JSON data structure
  console.log("Document updated with AI response");
}
...
const documentPathMap = {
  ...
  "startDate": ["Employee Contract.DATE OF COMMENCEMENT AND CONTINUOUS EMPLOYMENT AND NOTICE PERIOD.date"],
  "continuousEmployment": ["Employee Contract.DATE OF COMMENCEMENT AND CONTINUOUS EMPLOYMENT AND NOTICE PERIOD.options1"],
  "previousEmployer": ["Employee Contract.DATE OF COMMENCEMENT AND CONTINUOUS EMPLOYMENT AND NOTICE PERIOD.options1"],
  "previousEmploymentDate": ["Employee Contract.DATE OF COMMENCEMENT AND CONTINUOUS EMPLOYMENT AND NOTICE PERIOD.options1"],
  "contractType": ["Employee Contract.DATE OF COMMENCEMENT AND CONTINUOUS EMPLOYMENT AND NOTICE PERIOD.options2"],
  "temporaryEndDate": ["Employee Contract.DATE OF COMMENCEMENT AND CONTINUOUS EMPLOYMENT AND NOTICE PERIOD.options2"],
  "temporaryNoticePeriod": ["Employee Contract.DATE OF COMMENCEMENT AND CONTINUOUS EMPLOYMENT AND NOTICE PERIOD.options2"],
  "fixedTermEndDate": ["Employee Contract.DATE OF COMMENCEMENT AND CONTINUOUS EMPLOYMENT AND NOTICE PERIOD.options2"],
  "fixedTermNoticePeriod": ["Employee Contract.DATE OF COMMENCEMENT AND CONTINUOUS EMPLOYMENT AND NOTICE PERIOD.options2"],
  "probationPeriod": ["Employee Contract.DATE OF COMMENCEMENT AND CONTINUOUS EMPLOYMENT AND NOTICE PERIOD.options2"],
...
function applyFormDataToFlatDocument(flatDoc, formData) {
  ...
  if (formData.employerName || formData.employerRegistration || formData.employerAddress) {
    ...
    updatedFlatDoc["Employee Contract.PARTIES.a"] = 
  ...
  if (formData.employeeName || formData.employeeAddress) {
    ...
    updatedFlatDoc["Employee Contract.PARTIES.b"] = 
      `${employeeName} of ${employeeAddress} ("you").`;
  }
  
  // Job title
  if (formData.jobTitle) {
    updatedFlatDoc["Employee Contract.DUTIES AND JOB TITLE.a"] = 
  ...
  if (formData.jobDutiesType === "General duties as determined by employer") {
    ...
    updatedFlatDoc["Employee Contract.DUTIES AND JOB TITLE.selected"] = 
      "Such duties and responsibilities as may be determined by the Employer from time to time.";
  } else if (formData.jobDutiesType === "Specific duties" && formData.specificDuties) {
    updatedFlatDoc["Employee Contract.DUTIES AND JOB TITLE.options"] = [
      "Such duties and responsibilities as may be determined by the Employer from time to time.",
  ...
  if (formData.startDate) {
    updatedFlatDoc["Employee Contract.DATE OF COMMENCEMENT AND CONTINUOUS EMPLOYMENT AND NOTICE PERIOD.date"] = formData.startDate;
  }
  
  // Continuous employment
  if (formData.continuousEmployment === "No") {
    updatedFlatDoc["Employee Contract.DATE OF COMMENCEMENT AND CONTINUOUS EMPLOYMENT AND NOTICE PERIOD.options1"] = [
    ...
    updatedFlatDoc["Employee Contract.DATE OF COMMENCEMENT AND CONTINUOUS EMPLOYMENT AND NOTICE PERIOD.selected1"] = 
      "No employment with a previous employer counts as part of your continuous employment.";
  } else if (formData.continuousEmployment === "Yes" && formData.previousEmployer && formData.previousEmploymentDate) {
    updatedFlatDoc["Employee Contract.DATE OF COMMENCEMENT AND CONTINUOUS EMPLOYMENT AND NOTICE PERIOD.options1"] = [
      "No employment with a previous employer counts as part of your continuous employment.",
  ...
  // Contract type
  if (formData.contractType === "Temporary" && formData.temporaryEndDate && formData.temporaryNoticePeriod) {
    ...
    updatedFlatDoc["Employee Contract.DATE OF COMMENCEMENT AND CONTINUOUS EMPLOYMENT AND NOTICE PERIOD.options2"] = [
      newValue,
    ...
    updatedFlatDoc["Employee Contract.DATE OF COMMENCEMENT AND CONTINUOUS EMPLOYMENT AND NOTICE PERIOD.selected2"] = newValue;
  } else if (formData.contractType === "Fixed term" && formData.fixedTermEndDate && formData.fixedTermNoticePeriod) {
    updatedFlatDoc["Employee Contract.DATE OF COMMENCEMENT AND CONTINUOUS EMPLOYMENT AND NOTICE PERIOD.options2"] = [
    ...
    updatedFlatDoc["Employee Contract.DATE OF COMMENCEMENT AND CONTINUOUS EMPLOYMENT AND NOTICE PERIOD.selected2"] = 
      updatedFlatDoc["Employee Contract.DATE OF COMMENCEMENT AND CONTINUOUS EMPLOYMENT AND NOTICE PERIOD.options2"][1];
  } else if (formData.contractType === "Open-ended" && formData.probationPeriod) {
    updatedFlatDoc["Employee Contract.DATE OF COMMENCEMENT AND CONTINUOUS EMPLOYMENT AND NOTICE PERIOD.options2"] = [
      "Temporary: Employment expected to continue until *[date]*; terminable by either party with *[number]* notice in writing; summary termination for gross misconduct.",
      "Fixed term: Employment will terminate on *[date]*; terminable by either party with *[number]* notice in writing; summary termination for gross misconduct.",
    ...
    updatedFlatDoc["Employee Contract.DATE OF COMMENCEMENT AND CONTINUOUS EMPLOYMENT AND NOTICE PERIOD.selected2"] = 
      updatedFlatDoc["Employee Contract.DATE OF COMMENCEMENT AND CONTINUOUS EMPLOYMENT AND NOTICE PERIOD.options2"][2];
  }
  
  // Working hours
  if (formData.workStartTime && formData.workEndTime && formData.lunchStartTime && formData.lunchEndTime) {
    updatedFlatDoc["Employee Contract.HOURS OF WORK.a"] = 
  ...
  if (formData.workLocation && formData.workAddress) {
    updatedFlatDoc["Employee Contract.PLACE OF WORK.text"] = 
  ...
  // Salary
  if (formData.salary) {
    updatedFlatDoc["Employee Contract.REMUNERATION.a"] = `Your salary is £${formData.salary} per year.`;
  }
  
  // Payment details
  if (formData.paymentFrequency && formData.paymentDate) {
    updatedFlatDoc["Employee Contract.REMUNERATION.b"] = `Paid ${formData.paymentFrequency.toLowerCase()} normally on ${formData.paymentDate}.`;
  }
  
  // Payment method
  if (formData.paymentMethod) {
    const method = formData.paymentMethod === "Other" ? "other" : formData.paymentMethod.toLowerCase();
    updatedFlatDoc["Employee Contract.REMUNERATION.c"] = `Payment will be made by ${method}.`;
  }
  ...
  } else if (formData.collectiveAgreement === "Yes" && formData.agreementDetails) {
    updatedFlatDoc["Employee Contract.COLLECTIVE AGREEMENTS.options"] = [
      "There are no collective agreements relevant to your employment.",
  ...
  if (formData.holidayDays) {
    updatedFlatDoc["Employee Contract.HOLIDAYS.a"] = `You are entitled to ${formData.holidayDays} days including statutory and public holidays per calendar year.`;
  }
  
  if (formData.holidayYearStart && formData.holidayYearEnd) {
    updatedFlatDoc["Employee Contract.HOLIDAYS.b"] = `Holiday year commences on ${formData.holidayYearStart} and finishes on ${formData.holidayYearEnd}.`;
  }
  ...
  } else if (formData.sickPayOption === "Occupational sick pay" && formData.occupationalSickDays) {
    updatedFlatDoc["Employee Contract.SICKNESS ABSENCE.options"] = [
      "Statutory Sick Pay after four days; no contractual pay.",
  ...
  } else if (formData.pensionOption === "Designated pension scheme" && formData.pensionName && formData.employerContribution && formData.employeeContribution) {
    updatedFlatDoc["Employee Contract.PENSION.options"] = [
      "No pension arrangements.",
  ...
  if (formData.governingLaw) {
    updatedFlatDoc["Employee Contract.JURISDICTION.a"] = 
      `This Agreement governed by ${formData.governingLaw} Law.`;
  }
  if (formData.jurisdictionCourts) {
    updatedFlatDoc["Employee Contract.JURISDICTION.b"] = 
...
function convertToHtml(document) {
  ...
  function processSection(key, value, level, path) {
    ...
    if (isMainSection) {
      ...
      html.push(
        `<div class="document-line ${sectionClass}" data-path="${currentPath}" style="margin-left: ${marginLeft + 20}px;">
    ...
    if (typeof value === "object" && value !== null) {
      ...
      if (Array.isArray(value) && key.toLowerCase().includes("options")) {
        ...
        const selectedValue = parentObject && parentObject[selectionKey] 
          ? parentObject[selectionKey] 
          : value[0];
        
        console.log(`Options at ${currentPath}, selection key: ${selectionKey}, value: ${selectedValue}`);
        ...
        Object.keys(value).forEach((subKey) => {
          ...
          if (subKey === "selected" || subKey === "selected1" || subKey === "selected2") {
            return;
          }
          ...
          if (subValue && typeof subValue === "object" && !Array.isArray(subValue)) {
            if (subValue.content !== undefined) {
              ...
              const shouldShowSubKey = !['content', 'title'].includes(subKey.toLowerCase());
              html.push(
                `<div class="document-line document-content" data-path="${currentPath}.${subKey}.content" style="margin-left: ${subMarginLeft}px;">
                  <span data-value-path="${currentPath}.${subKey}.content">
                    ${shouldShowSubKey ? `<strong>${subKey}:</strong> ` : ''}${subValue.content}
                  </span>
                </div>`
              );
            } else {
          ...
          } else if (Array.isArray(subValue) && (subKey.toLowerCase().includes("options") || subKey.toLowerCase().includes("type"))) {
            // Handle option array with possible selection
            const subSelectionKey = subKey === "options1" ? "selected1" : 
                                   subKey === "options2" ? "selected2" : "selected";
  ...
  document
    ...
    .forEach((input) => {
      input.addEventListener("input", function() {
        ...
        if (this.id === "jobDutiesType" || this.id === "continuousEmployment" || 
            this.id === "contractType" || this.id === "overtimeOption" || 
            this.id === "collectiveAgreement" || this.id === "sickPayOption" || 
...
function createInputElement(key, data) {
  ...
  switch (data.type) {
    case "textarea":
      return `<textarea id="${key}" class="form-textarea" ${affectedPaths}>${data.default || ""}</textarea>`;
    case "date":
      return `<input type="date" id="${key}" ${affectedPaths}>`;
    default:
      return `<input type="text" id="${key}" value="${data.default || ""}" ${affectedPaths}>`;
  }
}
...
console.log("DOM elements ready:", !!document.getElementById("documentPreview"), !!document.getElementById("keyContainer"));
