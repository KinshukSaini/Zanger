...
const documentQuestions = {
  ...
  step4: {
    ...
    restrictedPowers: {
      question: "Enter restrictions on Managing Shareholder powers (separate by commas)",
      type: "textarea"
    }
  },
  step5: {
    ...
    distributionFrequency: {
      ...
      type: "select",
      options: ["weekly", "bi-weekly", "semi-monthly", "monthly", "quarterly", "semi-annually", "annually"]
    },
...
const documentPathMap = {
  ...
  "shareholder1.signatureDate": ["Investment Agreement.SIGNATURES.signature_blocks.0.date_line"],
  "shareholder2.signatureDate": ["Investment Agreement.SIGNATURES.signature_blocks.1.date_line"],
  "shareholder3.signatureDate": ["Investment Agreement.SIGNATURES.signature_blocks.2.date_line"],
  "shareholder4.signatureDate": ["Investment Agreement.SIGNATURES.signature_blocks.3.date_line"],
  
  // Shareholders info
  "shareholderCount": ["Investment Agreement.INTRODUCTION.parties.shareholders"],
  "shareholder1.name": ["Investment Agreement.INTRODUCTION.parties.shareholders", "Investment Agreement.Share Subject to This Agreement.1.a"],
  "shareholder1.address": ["Investment Agreement.INTRODUCTION.parties.shareholders"],
  "shareholder1.shares": ["Investment Agreement.Share Subject to This Agreement.1.a"],
  "shareholder2.name": ["Investment Agreement.INTRODUCTION.parties.shareholders", "Investment Agreement.Share Subject to This Agreement.1.b"],
  "shareholder2.address": ["Investment Agreement.INTRODUCTION.parties.shareholders"],
  "shareholder2.shares": ["Investment Agreement.Share Subject to This Agreement.1.b"],
  "shareholder3.name": ["Investment Agreement.INTRODUCTION.parties.shareholders", "Investment Agreement.Share Subject to This Agreement.1.c"],
  "shareholder3.address": ["Investment Agreement.INTRODUCTION.parties.shareholders"],
  "shareholder3.shares": ["Investment Agreement.Share Subject to This Agreement.1.c"],
  "shareholder4.name": ["Investment Agreement.INTRODUCTION.parties.shareholders", "Investment Agreement.Share Subject to This Agreement.1.d"],
  "shareholder4.address": ["Investment Agreement.INTRODUCTION.parties.shareholders"],
  "shareholder4.shares": ["Investment Agreement.Share Subject to This Agreement.1.d"],
  
  // Management info
  "managingShareholder": ["Investment Agreement.Management and Control.6.a", "Investment Agreement.Management and Control.6.b"],
  ...
  // Legal terms
  "noncompeteYears": ["Investment Agreement.Noncompetition and Trade Secrets.22.b"],
...
document.addEventListener("DOMContentLoaded", async function () {
  ...
  if (!window.currentDocument) {
    ...
    try {
      ...
      console.log("Loaded investment-agreement.json into window.currentDocument");
    } catch (e) {
...
function convertToHtml(document) {
  ...
  if (documentTitle) {
    ...
    if (mainContent["INTRODUCTION"]) {
      ...
      if (introValue.parties) {
        ...
        if (introValue.parties.shareholders) {
          ...
          html.push(`<div class="shareholders-section" data-path="${shareholdersPath}" style="margin-left: 40px;">`);
    ...
    sectionOrder
      .filter(section => section !== "INVESTMENT AGREEMENT" && section !== "INTRODUCTION")
      .forEach((section) => {
          ...
          if (subValue.content !== undefined) {
              ...
              html.push(
                `<div class="document-line document-content" data-path="${subPath}.content" style="margin-left: ${subMarginLeft}px;">
                  <span data-value-path="${subPath}.content">
                    ${subKey !== 'content' ? `<strong>${subKey}:</strong> ` : ''} ${subValue.content}
                  </span>
                </div>`
              );
            } 
            // For array content
              ...
              subValue.content.forEach((item, idx) => {
                ...
                html.push(
                  `<div class="document-line document-content" data-value-path="${itemPath}" style="margin-left: ${subMarginLeft + 20}px;">
                    ${item}
                  </div>`
                );
              });
            ...
            for (const propKey in subValue) {
              if (propKey !== 'content' && propKey !== 'title') {
                ...
                if (typeof propValue === 'string') {
                  html.push(
                    `<div class="document-line document-content" data-value-path="${propPath}" style="margin-left: ${subMarginLeft + 20}px;">
  ...
  function handleSignatureBlocks(path, blocks, marginLeft) {
    html.push(`<div class="signature-section" data-path="${path}" style="margin-left: ${marginLeft}px;">`);
    
    blocks.forEach((block, index) => {
      const blockPath = `${path}.${index}`;
      html.push(`<div class="signature-block" data-path="${blockPath}" style="margin-left: ${marginLeft + 20}px;">`);
      
      if (block.signature_line) {
        html.push(
          `<div class="document-line" data-value-path="${blockPath}.signature_line" style="margin-left: ${marginLeft + 40}px;">
      ...
      if (block.name_line) {
        html.push(
          `<div class="document-line" data-value-path="${blockPath}.name_line" style="margin-left: ${marginLeft + 40}px;">
            ${block.name_line}
          </div>`
        );
      }
      ...
      if (block.date_line) {
        html.push(
          `<div class="document-line" data-value-path="${blockPath}.date_line" style="margin-left: ${marginLeft + 40}px;">
            ${block.date_line}
          </div>`
        );
      }
    ...
    input.addEventListener("blur", function() {
      setTimeout(() => {
        if (!document.activeElement ||
            !document.activeElement.matches("#keyContainer input, #keyContainer select, #keyContainer textarea")) {
          clearHighlights();
        }
      }, 100);
    });
...
function createQuestionField(key, data, sectionClass = "") {
  ...
  if (data.showIf) {
    ...
    visibilityAttr = `data-show-conditions='${JSON.stringify(conditions)}' style="display: none;"`;
...
function updateVisibleFields() {
  console.log("Updating visible fields. Current formDataStore:", JSON.stringify(formDataStore));
  ...
  document.querySelectorAll("[data-show-conditions]").forEach(field => {
    ...
    const fieldId = field.querySelector('input, select, textarea')?.id || 'group-' + field.id;
    ...
    conditions.forEach(condition => {
      ...
      console.log(`  Condition: ${condition.condition}, Required Values: ${condition.values}, Actual Value: ${controlValue}`);
    ...
    if (oldDisplay !== field.style.display) {
      console.log(`  Changed display for ${fieldId} from ${oldDisplay} to ${field.style.display}`);
    }
...
function applyFormDataToFlatDocument(flatDoc, formData) {
  ...
  const documentTitle = Object.keys(window.currentDocument)[0] || "Investment Agreement";

  // --- INTRODUCTION Section ---
  const formattedDate = formData.date ? formatDate(formData.date) : '*[INSERT DATE]*';
  updatedFlatDoc[`${documentTitle}.INTRODUCTION.content`] = `This Investment Agreement ("Agreement") is made and entered into as of ${formattedDate}, by and between:`;
  
  // INVESTMENT AGREEMENT place
  if (formData.place) {
    updatedFlatDoc[`${documentTitle}.INVESTMENT AGREEMENT.place`] = formData.place;
  }
  ...
  updatedFlatDoc[`${documentTitle}.INTRODUCTION.parties.shareholders`] = shareholders;
  ...
  if (formData.companyName || formData.companyAddress) {
    ...
    updatedFlatDoc[`${documentTitle}.INTRODUCTION.parties.company`] = `${companyName} whose official address is ${companyAddress} (the "Company")`;
  }
  ...
  if (formData.managingShareholder) {
    ...
    updatedFlatDoc[`${documentTitle}.Management and Control.6.a`] = `Except as set forth in this section, ${name}, or his/her duly appointed successor (the "Managing Shareholder") shall manage, control, and operate the business and affairs of the Company as President and General Manager without any further action, or approval by the Shareholders or the Board.`;
    updatedFlatDoc[`${documentTitle}.Management and Control.6.b`] = `The Managing Shareholder may be changed from time to time with the Consent of the Shareholders subject to the terms of any employment agreement between the Company and ${name}.`;
  }
  
  // Officer positions
  if (formData.president) {
    updatedFlatDoc[`${documentTitle}.Management and Control.10.b`] = `President: ${formData.president}`;
  }
  if (formData.vicePresident) {
    updatedFlatDoc[`${documentTitle}.Management and Control.10.c`] = `Vice President: ${formData.vicePresident}`;
  }
  if (formData.secretary) {
    updatedFlatDoc[`${documentTitle}.Management and Control.10.d`] = `Secretary: ${formData.secretary}`;
  }
  if (formData.treasurer) {
    updatedFlatDoc[`${documentTitle}.Management and Control.10.e`] = `Treasurer: ${formData.treasurer}`;
  }
  ...
  if (formData.restrictedPowers) {
    ...
    for (let i = 1; i <= 4; i++) {
      ...
      if (updatedFlatDoc[`${documentTitle}.Management and Control.6.c.${romanNum}`]) {
        delete updatedFlatDoc[`${documentTitle}.Management and Control.6.c.${romanNum}`];
      }
    }
    
    // Add new powers
    const powers = formData.restrictedPowers.split(',').map(p => p.trim()).filter(p => p);
    powers.forEach((power, index) => {
      if (index < 4) {
        const romanNum = ['i', 'ii', 'iii', 'iv'][index];
        updatedFlatDoc[`${documentTitle}.Management and Control.6.c.${romanNum}`] = power;
      }
    });
  }
  ...
  if (formData.terminationSharePrice || formData.terminationAmountLimit) {
    ...
    updatedFlatDoc[`${documentTitle}.Management and Control.11.c`] = `In the event of any such termination, the terminated Shareholder agrees to sell to the Company, and the Company agrees to purchase, in proportion to the shares of the Company then owned by them, the shares of the Company then owned by the terminated Shareholder at a purchase price of $${price} per share, or $${limit}, whichever is less.`;
  }
  ...
  const defaultSignatureDate = formData.signatureDate ? formatDate(formData.signatureDate) : '*[INSERT DATE]*';
  ...
  updatedFlatDoc[`${documentTitle}.SIGNATURES.signature_blocks`] = signatureBlocks;
