...
function initializeDocumentTemplate() {
  if (window.currentDocument) {
    ...
    console.error("window.currentDocument is not defined for licensing agreement.");
...
const documentQuestions = {
  ...
  step5: {
    ...
    disputeResolution: {
        ...
        type: "select",
        options: ["Court Litigation", "Binding Arbitration", "Mediation", "Mediation then binding arbitration"]
    },
    arbitrationRules: {
        question: "Arbitration Rules (if applicable, e.g., AAA, JAMS):",
        type: "text",
        showIf: "disputeResolution=Binding Arbitration,Mediation then binding arbitration" // Example of multiple values
    },
    amendmentFrequency: {
        question: "How frequently can fees be amended?",
        type: "select",
        options: ["monthly", "quarterly", "semi-annually", "annually", "bi-annually", "every 3 years", "every 5 years"]
    }
  }
};
...
const documentPathMap = {
  ...
  considerationAdditionalProvisions: ["Licensing Agreement.Consideration.4.content"],
  ...
  arbitrationRules: ["Licensing Agreement.Dispute Resolution.56.b", "Licensing Agreement.Dispute Resolution.56.d"], // Needs logic to pick correct path or update template
...
function applyFormDataToFlatDocument(flatDoc, formData) {
  ...
  if (formData.agreementDate || formData.ownerName || formData.userName) {
    const introDate = formData.agreementDate ? formatDate(formData.agreementDate) : "*[INSERT DATE]*";
    updatedFlatDoc[`${docTitle}.INTRODUCTION.content`] = `This License Agreement ("Agreement") is entered into on the ${introDate} between:`;
  }
  if (formData.ownerName || formData.ownerAddress) {
    ...
    updatedFlatDoc[`${docTitle}.INTRODUCTION.parties.owner`] = `${ownerName} located at ${ownerAddress} ("Owner")`;
  }
  if (formData.userName || formData.userAddress) {
    ...
    updatedFlatDoc[`${docTitle}.INTRODUCTION.parties.user`] = `${userName} located at ${userAddress} ("User")`;
  }
  if (formData.agreementPlace) {
    updatedFlatDoc[`${docTitle}.LICENSING AGREEMENT.place`] = formData.agreementPlace || "*[PLACE]*";
  }


  // LICENSE SECTION
  if (formData.licenseType) { // Removed licensedIpName1 from condition
      const licenseTypeStr = formData.licenseType === "Exclusive" ? "an exclusive" : "a non-exclusive";
      updatedFlatDoc[`${docTitle}.License.1.content`] = `The Owner hereby grants to the User ${licenseTypeStr} license to use the following items of intellectual property (the "Licensed IP"):`;
  } else if (!updatedFlatDoc[`${docTitle}.License.1.content`]) { // Ensure default if not in form & not in template
      updatedFlatDoc[`${docTitle}.License.1.content`] = `The Owner hereby grants to the User *[an exclusive/a non-exclusive]* license to use the following items of intellectual property (the "Licensed IP"):`;
  }
  ...
  for (const keyInFlatDoc in updatedFlatDoc) {
    if (keyInFlatDoc.startsWith(license1ItemPrefix)) {
      ...
      if (firstPartOfSubKey.length === 1 && firstPartOfSubKey >= 'a' && firstPartOfSubKey <= 'z') {
  ...
  for (let i = 0; i < numberOfIpItems; i++) {
    ...
    const itemName = formData[`licensedIpName_${i + 1}`] || "*[NAME/TITLE OF IP]*";
    const itemDesc = formData[`licensedIpDesc_${i + 1}`] || "*[DESCRIPTION OF IP]*";

    updatedFlatDoc[`${license1ItemPrefix}${itemKeyChar}.content`] = `${itemName}:`;
  ...
  if (formData.licensePurpose || formData.territoryType) {
      ...
      let territoryStr = "*[in the following regions: [INSERT REGIONS] **OR** worldwide]*";
      if (formData.territoryType === "Worldwide") {
          territoryStr = "worldwide";
      } else if (formData.territoryType === "Specific Regions" && formData.specificRegions) {
      ...
      updatedFlatDoc[`${docTitle}.License.2.content`] = `Solely for the limited purposes of ${purpose}. User is authorized to use the Licensed IP ${territoryStr} (the "Territory").`;
  }
  ...
  if (updatedFlatDoc[`${docTitle}.Consideration.3`]) { // Delete the old object key itself if it exists
    delete updatedFlatDoc[`${docTitle}.Consideration.3`];
  }

  for (let i = 0; i < numberOfPayments; i++) {
    const paymentType = formData[`paymentType_${i + 1}`] || "*[INSERT TYPE OF PAYMENT]*";
  ...
  if (formData.considerationAdditionalProvisions) {
      updatedFlatDoc[`${docTitle}.Consideration.4.content`] = formData.considerationAdditionalProvisions || "*[INSERT ADDITIONAL PROVISIONS]*";
  }
  if (formData.paymentGracePeriodDays || formData.latePaymentInterest || formData.latePaymentFee) {
      const graceDays = formData.paymentGracePeriodDays || "*[DAYS]*";
      let lateCharges = [];
      if (formData.latePaymentInterest) lateCharges.push(`Interest of ${formData.latePaymentInterest}% per month charged.`);
      if (formData.latePaymentFee) lateCharges.push(`A late fee of $${formData.latePaymentFee} for each month past the due date.`);
      const lateChargesStr = lateCharges.length > 0 ? lateCharges.join(" ") : "*[INSERT THOSE THAT APPLY]*";

      updatedFlatDoc[`${docTitle}.Consideration.5.content`] = `Payment shall be made within ${graceDays} days of the due date. If any payment is not made within *[DAYS]* days after the due date, the Owner may charge the following: ${lateChargesStr}`;
      ...
      if (formData.latePaymentInterest) {
        updatedFlatDoc[`${docTitle}.Consideration.5.a`] = `Interest of ${formData.latePaymentInterest || "[PERCENTAGE]"}% per month charged.`;
      }
      if (formData.latePaymentFee) {
         updatedFlatDoc[`${docTitle}.Consideration.5.b`] = `A late fee of $${formData.latePaymentFee || "[AMOUNT]"} for each month past the due date.`;
      }
  }
  ...
  if (formData.initialTermYears) {
      updatedFlatDoc[`${docTitle}.Term.44.content`] = `This Agreement will commence on the Effective Date and will continue in full force and effect for an initial period of ${formData.initialTermYears || "*[YEAR(S)]*"} year(s).`;
  }
  ...
  if (formData.terminationCurePeriodDays) {
      updatedFlatDoc[`${docTitle}.Termination.46.content`] = `Either party may terminate this Agreement immediately upon delivery of written notice to the other party, clearly specifying the grounds for termination if the other party commits a material breach of its obligations under this Agreement and fails to cure the breach within ${formData.terminationCurePeriodDays || "*[DAYS]*"} days after written notice of the breach is received by the breaching party.`;
  }

  // GOVERNING LAW & DISPUTE RESOLUTION
  if (formData.governingLaw) {
      updatedFlatDoc[`${docTitle}.Governing Law.55.content`] = `This Agreement shall be governed under the ${formData.governingLaw || "[GOVERNING LAW]"}.`;
  }
  if (formData.disputeResolution) {
    let disputeText = "Any dispute arising from this Agreement shall be resolved through: ";
    const method = formData.disputeResolution;
    const arbitrationRules = formData.arbitrationRules || "*[INSERT ARBITRATION RULES]*"; 

    // Clear all existing sub-options first
    // This is important so we don't render unused options
    if (updatedFlatDoc[`${docTitle}.Dispute Resolution.56.a`]) delete updatedFlatDoc[`${docTitle}.Dispute Resolution.56.a`];
    if (updatedFlatDoc[`${docTitle}.Dispute Resolution.56.b`]) delete updatedFlatDoc[`${docTitle}.Dispute Resolution.56.b`];
    if (updatedFlatDoc[`${docTitle}.Dispute Resolution.56.c`]) delete updatedFlatDoc[`${docTitle}.Dispute Resolution.56.c`];
    if (updatedFlatDoc[`${docTitle}.Dispute Resolution.56.d`]) delete updatedFlatDoc[`${docTitle}.Dispute Resolution.56.d`];

    if (method === "Court Litigation") {
        disputeText += "Court Litigation: if either Party brings legal action, the prevailing party will be entitled to recover from the other party, any legal expenses incurred in relation to the claim.";
    ...
    updatedFlatDoc[`${docTitle}.Dispute Resolution.56.content`] = "Any dispute arising from this Agreement shall be resolved through:";
}

  // AMENDMENTS SECTION
  if (formData.amendmentFrequency) {
      updatedFlatDoc[`${docTitle}.Amendments.60.c`] = `and to amend the amount of fees, but no more frequently than ${formData.amendmentFrequency}.`;
  }
...
function highlightDocumentSection(questionId) {
  ...
  if (!paths && questionId.startsWith("licensedIp")) { // e.g., licensedIpName_1, licensedIpDesc_1
...
function renderDynamicIpQuestions(count) {
  ...
  document.querySelectorAll("#dynamicIpItemsContainer input, #dynamicIpItemsContainer textarea")
    .forEach(input => {
...
function showQuestionnaire() {
  ...
  document.querySelectorAll("#keyContainer input, #keyContainer select, #keyContainer textarea")
    .forEach(input => {
      if (input.id.startsWith("licensedIpName_") || input.id.startsWith("licensedIpDesc_")) {
          return; // Skip dynamic ones, they have listeners from renderDynamicIpQuestions
      }

      // Restore value for static fields
      if (formDataStore[input.id] && input.id !== 'numberOfIpItems') { // numberOfIpItems already handled
          input.value = formDataStore[input.id];
      }
      input.addEventListener("input", function() {
        formDataStore[this.id] = this.value;
        // Handle conditional visibility if this field controls others
        if (this.id === 'territoryType' || this.id === 'renewalOption' || this.id === 'disputeResolution') {
        ...
        if (this.id === 'numberOfIpItems') {
            ...
            if (count >= minCount) { // Ensure count is not less than min
        ...
        if (this.id === 'numberOfPayments') {
            let count = parseInt(this.value, 10) || 0;
            const minPaymentCount = documentQuestions.step3.numberOfPayments.min || 1;
      ...
      if (input.tagName === 'SELECT' && (input.id === 'territoryType' || input.id === 'renewalOption' || input.id === 'disputeResolution')) {
  ...
  Object.keys(formDataStore).forEach(key => {
    ...
    if (inputElement && !(key.startsWith("licensedIpName_") || key.startsWith("licensedIpDesc_")) && key !== 'numberOfIpItems') {
      inputElement.value = formDataStore[key];
      if (inputElement.tagName === 'SELECT' && (inputElement.id === 'territoryType' || inputElement.id === 'renewalOption' || inputElement.id === 'disputeResolution')) {
...
function createQuestionsHTML(stepData, stepKey) { // Added stepKey
  let html = "";
  for (const [key, data] of Object.entries(stepData)) {
    if (key === "title") continue;
    if (typeof data === "object" && data.question) { // Single question
...
function createQuestionField(key, data) {
  ...
  if (data.showIf) {
    ...
    visibilityAttr = `data-show-if-key="${conditionKey}" data-show-if-values="${conditionValues.join(',')}" style="display: none;"`;
  }
  return `
...
function handleConditionalFieldChange(controllerElement) {
    ...
    document.querySelectorAll(`[data-show-if-key="${controllerKey}"]`).forEach(dependentField => {
        const showIfValues = dependentField.getAttribute('data-show-if-values').split(',');
        if (showIfValues.includes(controllerValue)) {
            ...
            const inputInside = dependentField.querySelector('input, select, textarea');
            if(inputInside) {
...
function convertToHtml(doc) {
  ...
  if (!docTitle || !doc[docTitle]) return "<p>Document data is missing or malformed.</p>";
  ...
  sectionOrder.forEach(sectionKey => {
    if (mainContent[sectionKey]) {
      ...
      htmlParts.push(`<div class="document-section" data-path="${sectionPath}">`); // data-path for the whole section
  ...
  if (typeof sectionData === 'string') {
    ...
    if (currentBasePath.endsWith('.Consideration.3')) {
      sectionData.forEach((paymentTerm, index) => {
        ...
        htmlParts.push(`<div class="document-line sub-item-title" style="margin-left: ${indent + 15}px;"><strong>Payment Term ${index + 1}:</strong></div>`);
        if (paymentTerm.type_content) {
          htmlParts.push(`<div class="document-line" style="margin-left: ${indent + 30}px;" data-value-path="${itemPath}.type_content">${paymentTerm.type_content}</div>`);
        }
        if (paymentTerm.due_date_content) {
          htmlParts.push(`<div class="document-line" style="margin-left: ${indent + 30}px;" data-value-path="${itemPath}.due_date_content">${paymentTerm.due_date_content}</div>`);
        }
        if (paymentTerm.amount_content) {
          htmlParts.push(`<div class="document-line" style="margin-left: ${indent + 30}px;" data-value-path="${itemPath}.amount_content">${paymentTerm.amount_content}</div>`);
        }
      });
    ...
    if (currentBasePath.endsWith('.INTRODUCTION') && sectionData.parties) {
        if (sectionData.content) { // Render the main content of INTRODUCTION first
        ...
        if (sectionData.parties.owner) htmlParts.push(`<div class="document-line" style="margin-left: ${indent}px;" data-value-path="${partiesPath}.owner">${sectionData.parties.owner}</div>`);
        if (sectionData.parties.user) htmlParts.push(`<div class="document-line" style="margin-left: ${indent}px;" data-value-path="${partiesPath}.user">${sectionData.parties.user}</div>`);
        if (sectionData.parties.agreement_intro) htmlParts.push(`<div class="document-line" style="margin-left: ${indent}px;" data-value-path="${partiesPath}.agreement_intro">${sectionData.parties.agreement_intro}</div>`);
        // Render other keys in INTRODUCTION if any, excluding 'parties' and 'content' already handled
        Object.keys(sectionData).forEach(key => {
            if (key !== 'parties' && key !== 'content') {
                 processSectionContent(sectionData[key], `${currentBasePath}.${key}`, htmlParts, level);
            }
        });
        return;
    }
    if (currentBasePath.endsWith('.SIGNATURES') && sectionData.signature_blocks) {
        if (sectionData.content) { // Render the main content of SIGNATURES first
             htmlParts.push(`<div class="document-line" style="margin-left: ${indent}px;" data-value-path="${currentBasePath}.content">${sectionData.content}</div>`);
        }
        ...
        if(sectionData.signature_blocks.owner) {
            htmlParts.push(`<div class="document-line" style="margin-left: ${indent}px;"><strong>Owner Signatures:</strong></div>`);
            sectionData.signature_blocks.owner.forEach((sig, i) => {
                htmlParts.push(`<div class="document-line" style="margin-left: ${indent + 15}px;" data-value-path="${sigBlockPath}.owner.${i}.signature_line">${sig.signature_line || '____________________'}</div>`);
                htmlParts.push(`<div class="document-line" style="margin-left: ${indent + 15}px;" data-value-path="${sigBlockPath}.owner.${i}.name_line">${sig.name_line || 'Name'}</div><br/>`);
            });
        }
        // ... (similar for user) ...
        Object.keys(sectionData).forEach(key => {
    ...
    if (currentBasePath.endsWith('.License.1')) {
      ...
      letterKeys.forEach(letter => {
        ...
        if (sectionData[letter].content) {
          htmlParts.push(`<div class="document-line" style="margin-left: ${indent + 15}px;" data-value-path="${itemPath}.content">${sectionData[letter].content}</div>`);
        }
        
        // IP description (i)
        if (sectionData[letter].i) {
          htmlParts.push(`<div class="document-line" style="margin-left: ${indent + 30}px;" data-value-path="${itemPath}.i">${sectionData[letter].i}</div>`);
        }
      });
      
      // Add any note or other non-letter keys at the end
      Object.keys(sectionData)
        .filter(key => !(key === 'content' || (key.length === 1 && key >= 'a' && key <= 'z')))
        .forEach(key => {
          processSectionContent(sectionData[key], `${currentBasePath}.${key}`, htmlParts, level + 1);
        });
    ...
    Object.keys(sectionData).sort().forEach(key => {
      ...
      if (typeof sectionData[key] === 'string') {
        ...
        const displayKey = (key === 'content' && Object.keys(sectionData).length === 1) ? '' : `<strong>${key}:</strong> `;
        htmlParts.push(`<div class="document-line" style="margin-left: ${indent}px;" data-value-path="${valuePath}">${displayKey}${sectionData[key]}</div>`);
      } else {
        // For nested objects/arrays, print the key as a sub-header (if not a list marker) and recurse
        if (!/^(i{1,3}|[a-z])$/.test(key) || typeof sectionData[key] !== 'string') {
             htmlParts.push(`<div class="document-line sub-item-title" style="margin-left: ${indent}px;" data-path="${valuePath}"><strong>${key}</strong></div>`); // Add data-path for sub-sections
        }
        processSectionContent(sectionData[key], valuePath, htmlParts, level + 1);
      }
    });
  }
}
...
document.addEventListener("DOMContentLoaded", async function () {
  ...
  if (!window.currentDocument || !window.currentDocument["Licensing Agreement"]) {
     console.warn("window.currentDocument for Licensing Agreement not found or malformed. Attempting to load from template.");
     ...
     try {
        const response = await fetch('../templates/licensing-agreement.json'); // Adjust path as needed
        if (!response.ok) throw new Error(`HTTP error! status: ${response.status}`);
        window.currentDocument = await response.json();
        console.log("Loaded licensing-agreement.json into window.currentDocument");
     } catch (e) {
  ...
  let initialPaymentCount = documentQuestions.step3.numberOfPayments.default || 1;
...
function renderDynamicPaymentQuestions(count) {
  ...
  for (let i = 1; i <= count; i++) {
    ...
    dynamicHTML += createQuestionField(`paymentType_${i}`, { question: `Type of Payment ${i}:`, type: "text", placeholder: "e.g., Royalty, Flat Fee" });
    dynamicHTML += createQuestionField(`paymentDueDate_${i}`, { question: `Payment ${i} Due Date:`, type: "text", placeholder: "e.g., Upon signing, Net 30" });
    dynamicHTML += createQuestionField(`paymentAmount_${i}`, { question: `Payment ${i} Amount ($):`, type: "text", placeholder: "e.g., 1000.00" });
  }
  container.innerHTML = dynamicHTML;

  document.querySelectorAll("#dynamicPaymentsContainer input") // Assuming only text inputs for now
    .forEach(input => {
      if (formDataStore[input.id]) {
          input.value = formDataStore[input.id];
      }
