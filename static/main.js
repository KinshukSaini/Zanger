  ...
  step2: {
    ...
    partnership: {
      name: {
        ...
        type: "text",
        showIf: "consultantType=Partnership",
      },
      address: {
...
function createQuestionsHTML(stepData) {
  ...
  // Add section identifier classes based on new party labels
...
function applyFormDataToFlatDocument(flatDoc, formData) {
  ...
  const documentTitle =
    Object.keys(window.currentDocument)[0] || "Consultancy Agreement";
  ...
  if (
    ...
    window.currentDocument[documentTitle]?.["PARTIES"]?.["1"]?.content
  ) {
    const party1Key = `${documentTitle}.PARTIES.1.content`;
    let party1Content = "";
    const existingParty1 =
      window.currentDocument[documentTitle]?.["PARTIES"]?.["1"]?.content || "";

    if (formData.consultantType === "Individual") {
      const name = formData.consultant_individual_name || "*[INDIVIDUAL NAME]*";
      const address = formData.consultant_individual_address || "*[address]*";
      party1Content = `${name} of ${address}`;
    } else if (formData.consultantType === "Company") {
      const name = formData.consultant_company_name || "*[COMPANY NAME]*";
      const regNumber =
        formData.consultant_company_regNumber || "*[registration number]*";
      const address = formData.consultant_company_address || "*[address]*";
      ...
      const name =
        formData.consultant_partnership_name || "*[PARTNERSHIP NAME]*";
      const address = formData.consultant_partnership_address || "*[address]*";
    ...
    // If no new data was provided, fall back on the existing party info
    updatedFlatDoc[party1Key] = party1Content || existingParty1;
  }

  // Update Client information (Party 2)
  if (formData.clientType) {
    const party2Key = `${documentTitle}.PARTIES.2.content`;
    let party2Content = "";

    if (formData.clientType === "Individual") {
      const name = formData.client_individual_name || "*[INDIVIDUAL NAME]*";
      const address = formData.client_individual_address || "*[address]*";
      party2Content = `${name} of ${address}`;
    } else if (formData.clientType === "Company") {
      const name = formData.client_company_name || "*[COMPANY NAME]*";
      const regNumber =
        formData.client_company_regNumber || "*[registration number]*";
      const address = formData.client_company_address || "*[address]*";
      ...
      const name = formData.client_partnership_name || "*[PARTNERSHIP NAME]*";
      const address = formData.client_partnership_address || "*[address]*";
    ...
    if (party2Content) {
      updatedFlatDoc[party2Key] = party2Content + ' (the "Client")';
    }
  }

  // Update Charges (Consideration)
  ...
  // [Optional] Additional mappings for other fields such as Services, Deliverables, Payments, etc.
  // You can add mappings below as needed.
  ...
  if (formData.consultantType) {
    ...
    if (formData.consultantType === "Individual") {
      ...
      const name = formData.consultant_company_name || "*[COMPANY NAME]*";
      const signatory =
        formData.consultant_company_signatory || "*[individual name]*";
      signatureContent = `SIGNED BY ${signatory} on *[...........], duly authorised for and on behalf of ${name}`;
    } else if (formData.consultantType === "Partnership") {
      const name =
        formData.consultant_partnership_name || "*[PARTNERSHIP NAME]*";
      const signatory =
        formData.consultant_partnership_signatory || "*[individual name]*";
      signatureContent = `SIGNED BY ${signatory} on *[...........], duly authorised for and on behalf of ${name}`;
    }
  ...
  if (formData.clientType) {
    ...
    if (formData.clientType === "Individual") {
      ...
      const name = formData.client_company_name || "*[COMPANY NAME]*";
      const signatory =
        formData.client_company_signatory || "*[individual name]*";
      signatureContent = `SIGNED BY ${signatory} on *[...........], duly authorised for and on behalf of ${name}`;
    } else if (formData.clientType === "Partnership") {
      const name = formData.client_partnership_name || "*[PARTNERSHIP NAME]*";
      const signatory =
        formData.client_partnership_signatory || "*[individual name]*";
      signatureContent = `SIGNED BY ${signatory} on *[...........], duly authorised for and on behalf of ${name}`;
    }
...
function updateDocumentWithFormData(formData) {
  // Use the current document as the starting point
  const currentDoc = window.currentDocument;
  const flatCurrentDoc = flattenObject(currentDoc);
  ...
  // Optionally update the global template so future updates merge correctly
  documentTemplate = JSON.parse(JSON.stringify(window.currentDocument));
...
function updateKeyEditor() {
  ...
  try {
    ...
    const html = paths
      .map(({ path, value }) => {
        // Update conditional check for DATE and PARTIES sections
        const isDateOrParties =
          path.startsWith("Consultancy Agreement.DATE") ||
          path.startsWith("Consultancy Agreement.1. PARTIES");
...
function editValue(path) {
  ...
  if (
    path.startsWith("Consultancy Agreement.DATE") ||
    path.startsWith("Consultancy Agreement.PARTIES")
  ) {
    saveButton.disabled = false;
  }
}
...
function splitPath(path) {
  ...
  // Instead of strict equality, check if the trimmed token includes "PARTIES"
  const specialIndex = parts.findIndex((token) =>
    token.trim().includes("PARTIES")
  );
  if (specialIndex === -1) {
