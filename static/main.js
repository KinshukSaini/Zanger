...
function resolveORChoices(text, selectedChoice = null) {
  ...
  // Convert markdown-style bold to HTML bold FIRST - ENHANCED
...
function convertToHtml(document) {
  ...
  function processSection(key, value, level, path, parentSection) {
    ...
    const isAgreementSubSection = parentSection === "AGREEMENT" && SECTION_PATTERN.test(key);
    ...
    if (typeof value === "object" && value !== null) {
      ...
      // Handle AGREEMENT section ordering
      let orderedKeys = keys;
      if (key === "AGREEMENT") {
        const actualKeys = Object.keys(value);
        orderedKeys = agreementSectionOrder
      ...
      orderedKeys.forEach((subKey) => {
        ...
        if (subValue && typeof subValue === "object") {
          ...
          if (subValue.selectedOption && subValue.content) {
            ...
            // Handle regular content with OR resolution - ENHANCED for bold text
            ...
            // Determine the appropriate label - FIXED for PARTIES section
            let labelToShow = '';
            if (parentSection === "PARTIES" && NUMBERED_PATTERN.test(subKey)) {
            ...
            Object.keys(subValue).forEach(innerKey => {
              if (innerKey !== 'content' && innerKey !== 'intro' && innerKey !== 'selectedOption') {
                ...
                // Handle definition terms - ENHANCED for bold text
            ...
            // Enhanced signature blocks formatting - ENHANCED for bold text
            html.push(
          ...
          // Handle string values - clean up brackets and OR choices - ENHANCED for bold text
  ...
  function processOptions(obj, currentPath, marginLeft) {
    ...
    ['a', 'b', 'c', 'd', 'e', 'f'].forEach(letter => {
      if (obj[letter]) {
        ...
        if (typeof value === 'object' && value.content) {
          // Handle complex options with suboptions - ENHANCED for bold text
          ...
          // Process suboptions (i, ii, iii, etc.) - ENHANCED for bold text
          ...
          // Handle death_clause specifically - ENHANCED for bold text
    ...
    // Handle additional list items - ENHANCED for bold text
    if (obj.additional) {
