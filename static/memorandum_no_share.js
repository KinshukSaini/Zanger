// Memorandum of Association (No Share Capital) document order configuration
const sectionOrder = [
  "COMPANY_NAME",
  "PREAMBLE", 
  "SUBSCRIBERS"
...
const documentQuestions = {
  step1: {
    ...
    companyName: {
      question: "Enter the company name (without share capital)",
      type: "text",
      placeholder: "e.g., ABC Community Interest Company"
    }
  },
  step2: {
...
let documentPathMap = {
  "companyName": ["Memorandum of Association (No Share Capital).COMPANY_NAME.content"]
};
...
function generateSubscriberQuestions(count) {
  ...
  for (let i = 1; i <= count; i++) {
    ...
    documentPathMap[`subscriber${i}Name`] = [`Memorandum of Association (No Share Capital).SUBSCRIBERS.subscribers.${i}.name`];
    documentPathMap[`subscriber${i}Authentication`] = [`Memorandum of Association (No Share Capital).SUBSCRIBERS.subscribers.${i}.authentication`];
  }
...
document.addEventListener("DOMContentLoaded", async function () {
  console.log("Memorandum (No Share Capital) document initialization started");
  if (!window.currentDocument) {
    console.error("No document found in window.currentDocument");
    window.currentDocument = { "Memorandum of Association (No Share Capital)": {} };
  }
  ...
  try {
    ...
    console.log("Memorandum (No Share Capital) document initialization completed");
  } catch (error) {
...
function convertToHtml(document) {
  ...
  html.push(
    `
    <h1 style="text-align: center;"> COMPANY NOT HAVING A SHARE CAPITAL </h1>
    `
  )
  ...
  function processSection(key, value, level, path) {
    ...
    if (key === "COMPANY_NAME") {
      // Company name as simple header (no "Memorandum of association of" prefix)
      html.push(
        `<div class="company-name" style="text-align: left; font-weight: normal; font-size: 14px; margin-bottom: 20px;">
          <span data-value-path="${currentPath}.content">Memorandum of association of ${value.content}</span>
        </div>`
      );
...
function updateDocumentStructureForSubscribers(count) {
  const documentTitle = Object.keys(window.currentDocument)[0] || "Memorandum of Association (No Share Capital)";
...
function applyFormDataToFlatDocument(flatDoc, formData) {
  ...
  // Company name (simple, no prefix)
  if (formData.companyName) {
    const companyNameKey = `${documentTitle}.COMPANY_NAME.content`;
    updatedFlatDoc[companyNameKey] = formData.companyName;
  }
...
function downloadWordDocx() {
  ...
  const html = `
    ...
    <head>
      <meta charset="utf-8">
      <title>Memorandum of Association (No Share Capital)</title>
      <style>
        body {
  ...
  link.href = url;
  link.download = "Memorandum_of_Association_No_Share_Capital.docx";
